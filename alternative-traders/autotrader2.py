...
class AutoTrader(BaseAutoTrader):
    ...
    def on_order_book_update_message(self, instrument: int, sequence_number: int, ask_prices: List[int],
        ...
        print("time is", (time.time() - self.start_time)*10)
        
        if abs(self.position - self.hedge_position) < 10:
            self.start_time = time.time()
        
        # if self.must_hedge or (time.time() - self.start_time >= 10):
        #     self.must_hedge = True
        #     self.start_time = time.time()
        #     if (self.hedge_position == -self.position):
        #         self.must_hedge = False
        #     else:
        #         if (len(self.ask_hedges)==0 and len(self.bid_hedges)==0 and len(self.bids)==0 and len(self.asks)==0):
